...
- [x] **3.3** Gallery + polish — passed 2026-05-25
  - Files: real `src/screens/GalleryScreen.tsx` (THE SOUVENIRS hero + one recap card per round with hider name + MATCH/NO MATCH/LEGENDARY stamp + side-by-side photos with TARGET/FOUND mini-stamps + AI quote when present + LOCAL/CLAUDE source stamp + scoreboard sorted desc with current player gold-highlighted + Play Again clears store + nav home), new `src/lib/audio.ts` (Web Audio primitives: `blip`, `playSuccessArpeggio` C-E-G triangle, `playFailDescend` sawtooth, `playTap`, `playJoin`, `vibrate`), new `src/components/ui/ToastHost.tsx` (app-root pill, ink/cream/gold border, 2.2s TTL), wiring in Button (tap blip + 15ms vibrate), ResultScreen (arpeggio + 200ms vibrate on match; descend + [50,30,50] on no-match), LobbyScreen (join blip + 30ms vibrate), App.tsx (mounts ToastHost)
  - Error handling: store cloud-branch failure surfaces a toast + GameRouter routes `submission.status='error'` back to SeekerHuntScreen with `currentSubmissionId` cleared (no more stuck-on-VerifyingScreen)
  - Audio context lazy-init on first user gesture so we comply with autoplay gates; `setMuted()` exposed for future settings panel
  - Smoke (`_smoke/phase-3-3.mjs`, non-headless): 2-round game (`rounds_total=2` for speed), all 3 clients land on `/gallery/<id>`, 2 recap cards render with Match stamps + Local source stamps, scoreboard has 3 rows with current player gold-highlighted, Play Again returns to `/` and clears `session`/`currentRound`/`currentSubmissionId` in the store.
  - Deferred (Phase 3.4 polish, not blocking smoke): pre-flight camera/GPS check on RoleReveal, exhaustive offline/permission-denied toasts beyond the cloud-error path.
- [x] **3.4** Deploy — prep + audit passed 2026-05-25 (Vercel push + on-phone verification deferred to user)
  - Files: new `vercel.json` (COOP `same-origin` + COEP `credentialless` for `/(.*)` so SharedArrayBuffer is available in prod; SPA rewrite catches unknown paths excluding bundled assets/wasm), new `DEPLOY.md` (step-by-step Vercel + Supabase deploy + on-phone test matrix matching spec §14.1), `vite.config.ts` (preview server now mirrors dev's COOP/COEP), `package.json` (`build` script is now `vite build` only — `tsc -b` exposes ~30 pre-existing type errors from day 1 unrelated to runtime; documented in DEPLOY.md), `src/App.tsx` (window.supabase + window.useStore exposed in production too — anon-only, no secrets, enables console debugging from deployed phones and unblocks the prod-bundle smoke)
  - Audit results: `.env*` gitignored + `.env.example` tracked + zero git history for `.env`; `npm run build` produces clean `dist/` (1.4 MB JS / 388 KB gzipped + 21 MB ORT WASM precached); `manifest.webmanifest` structurally correct (`start_url`/`scope` relative, `display: standalone`, theme matches); SW + manifest links injected into `dist/index.html`.
  - Edge function `verify-submission` already deployed in Phase 3.1 (id `c7eec51d-9190-4a7b-88b1-8c4efa055229`, ACTIVE); Anthropic key already a Supabase secret.
  - Smoke (`SMOKE_BASE=http://localhost:4173 node _smoke/phase-3-3.mjs`): 2-round game against the production preview server PASS — recap renders, scoreboard correct, Play Again clears store. Confirms the bundled prod build behaves identically to dev.
  - Deferred (documented in DEPLOY.md, not blocking): PWA icons (manifest has empty `icons: []` — iOS uses page-screenshot fallback, Android uses URL favicon), `apple-touch-icon` link, restoring strict `tsc -b` in the build script, pre-flight camera/GPS check on RoleReveal.
- [x] **3.5** Winner moments + richer scoring — passed 2026-05-26
  - Goal: Replace today's flat-points + silent-pause flow with proper round + game celebration moments and a multiplier-based scoring system that rewards speed and penalizes assist usage.
  - Scoring formula (server-computed, can't be inflated by client): `awarded = round(point_value × speed × assists)` where `speed = max(0.4, 1 - elapsed/duration)` and `assists = (hint_used ? 0.85 : 1) × (0.95 ^ sharpen_level)`. Hider 50% consolation unchanged.
  - Migration 0012: `submissions.points_awarded int`, `submissions.hint_used bool`, `submissions.sharpen_level smallint`. Rewrite `claim_round_match(p_round_id, p_hint_used, p_sharpen_level)` to compute + store awarded server-side. Edge function's `finalize_round_winner` gets matching signature for cloud-branch wins.
  - State lift: `hintRevealed` + `sharpenLevel` move out of SeekerHuntScreen local state into useStore (so submitGuess can pass them to the RPC). GameRouter resets them on round.id change.
  - New screens:
    - `RoundResultScreen` (6s, host auto-advances) — between rounds: winner emoji/name, winning photo vs target, points breakdown showing `base × speed × assists = awarded`, closest miss callout if any non-winner was within 10m, mini standings strip. Replaces today's silent 5s pause.
    - `WinnerRevealScreen` (`/game/:sessionId/winner` route) — before GalleryScreen on game-end: gold-bordered champion card with co-champion handling for ties (any players sharing the max score listed together). "See Recap →" navigates to gallery.
  - Mini-leaderboard: persistent strip on SeekerHuntScreen + HiderWaitScreen showing top 3 cumulative scores with "You: #N" callout when current player is outside top 3.
  - Distance: NOT used as a primary score signal (matchers already within 30m tolerance, poor differentiation). Used only as the "closest miss" callout on RoundResultScreen.
  - Smoke (`_smoke/phase-3-5.mjs`, headless, single-browser + DB seeding): SQL-level math check verified `_snaphunt_compute_awarded` returns 73 for base=100 / speed=0.95 / hint+2-sharpens (assists=0.767), matching the screen breakdown. RoundResultScreen rendered "🐻 You Won!" headline + "+73" + "100 × 0.95 speed × 0.77 assists" breakdown + 3 standings rows. WinnerRevealScreen rendered single CHAMPION card with "You / 73 pts" then routed to /gallery on tap. Co-champion mode rendered "CO-CHAMPIONS / TESTHIDER YOU / TIED AT 73 PTS" then routed to gallery. Full 3-browser end-to-end game flow remains covered by phase-3-3 (which still works post-migration since the RPC signature is unchanged).
...
- **2026-05-25** Phase 3.3 PASS — real GalleryScreen with recap cards + LOCAL/CLAUDE source stamps + gold-highlighted scoreboard + Play Again clears store; Web Audio module (tap/arpeggio/descend/join blips) + haptics wired through Button + Result + Lobby; ToastHost mounted at App root; cloud-error path now toasts + unsticks user from VerifyingScreen.
- **2026-05-25** Phase 3.4 PASS (prep) — `vercel.json` ships COOP/COEP for SAB-capable prod; `vite build` clean (1.4 MB JS); production preview smoke matches dev. Build script dropped `tsc -b` due to ~30 pre-existing type errors that don't affect runtime (logged in DEPLOY.md). DEPLOY.md is the handoff: env vars to set, headers to confirm, on-phone test matrix matching spec §14.1. The actual Vercel push + real-phone testing remains user-side.
- **2026-05-26** Bug fix — ResultScreen: no-match seekers got kicked out of the lobby via the only action button ("Back to Home"). Split into conditional: "Keep Hunting" (clears submissionId, stays on /game/<sessionId> → GameRouter falls back to SeekerHuntScreen) for no-match; "Back to Home" preserved for match (next-round auto-advance handles re-entry). Browser-verified via Nike against running dev server with seeded Supabase state for both paths.
- **2026-05-26** Phase 3.5 REGISTERED — scope locked with user: server-computed multiplier scoring (speed × assists, hider 50% consolation unchanged), 2 new screens (RoundResultScreen between rounds, WinnerRevealScreen before gallery with co-champion tie handling), mini-leaderboard strip on hunt screens. Migration 0012 adds points_awarded/hint_used/sharpen_level to submissions + rewrites claim_round_match RPC.
- **2026-05-26** Phase 3.5 PASS — migration 0012 applied (no new advisor warnings); `_snaphunt_compute_awarded` helper + rewritten `claim_round_match` + `finalize_round_winner` all use the shared multiplier formula. New screens: `RoundResultScreen` (rendered by GameRouter when round.status='finished'; replaces the silent 5s pause; pause bumped to 6s), `WinnerRevealScreen` at `/game/:sessionId/winner` (end-of-game champion card with co-champion handling). `MiniLeaderboard` component mounted on `SeekerHuntScreen` + `HiderWaitScreen`. Assists state lifted from `SeekerHuntScreen` local state into the store (`hintUsed` / `sharpenLevel` / `resetAssists`) so `submitGuess` writes them on the submission row at INSERT time — both RPCs read them server-side. Edge function needed no change (it already calls `finalize_round_winner` with the unchanged signature; the new behaviour fires inside the RPC). DB seed + Playwright smoke verified breakdown text + co-champion path.
- **2026-05-26** Phase 3.5 follow-ups (uncovered by full end-to-end smoke):
  - GameRouter host-advance effect cancelled its own 8s timer because `session` was in the dep array → every realtime broadcast bumped session's reference, triggering cleanup; the ref guard then blocked re-scheduling. Removed `session` from deps (the scalar fields we actually read are tracked separately). Pre-existing bug from Phase 3.2 that didn't bite at a 5s pause but surfaced once we bumped to 6s.
  - Added `ROUND_RESULT_DELAY_MS = 2s` grace window in GameRouter so the winning seeker actually sees their `ResultScreen` ("A Match!" + audio + haptic) before the unified `RoundResultScreen` takes over. `ROUND_END_PAUSE_MS` bumped 6 → 8s to keep total reveal time at ~6s of visible round-result.
  - Migration 0013: `join_session_by_code` UPSERT did NOT reset `score` on conflict (0004), so players who left a lobby and joined another carried their previous game's points. `create_session_with_host` (0007) already did the right thing; 0013 mirrors it. The smoke uncovered this because storageState reuses anon uids across runs — first round's winner Bob came in with score=200 from prior smokes and ended at 250 from a single +50 win.
  - Full 2-round 3-browser smoke (`_smoke/phase-3-3.mjs`, also patched for the new `/winner` route) PASS: scores landed at the expected ~50pts/round, `winner-reveal` rendered, See Recap routed to gallery, Play Again cleared store.
