  ...
  return (
    <main className="flex h-full w-full flex-col bg-void text-smoke">
      ...
      <div className="flex flex-1 flex-col px-5 pt-3 pb-5 overflow-y-auto">
        ...
        >
          {previewUrl ? (
            ...
            <div className="px-8 text-center">
              <div className="font-display text-[64px] leading-none opacity-30">💣</div>
              <p className="mt-3 font-mono text-[11px] uppercase tracking-[0.15em] text-ash/70 leading-relaxed">
        ...
        {/* Timer selection */}
        <div className="mb-4">
          <span className="block font-mono text-[10px] uppercase tracking-[0.25em] text-ash mb-2">
            Bomb Timer
          </span>
          <div className="grid grid-cols-4 gap-2">
            {BOMB_TIMER_OPTIONS.map(({ label, seconds }) => (
              <button
                ...
                className={[
                  'rounded-[2px] border-2 py-3 text-center font-display text-[18px] tracking-[0.04em] transition-all',
        ...
        {error && (
          <div className="mb-3 rounded-[2px] border border-threat/50 bg-threat/10 px-3 py-2 font-mono text-[11px] text-threat">
            {error}
          </div>
        )}
        {submitting && (
          <div className="mb-3 rounded-[2px] border border-rim bg-surface px-3 py-2 font-mono text-[11px] uppercase tracking-[0.15em] text-ash animate-blink">
        ...
        <div className="grid grid-cols-2 gap-2.5 mt-auto">
          <PhotoCapture
            ...
            buttonProps={{
              className: 'flex w-full items-center justify-center gap-2 rounded-[2px] border-2 border-rim bg-surface px-4 py-[17px] font-display text-[16px] text-smoke transition-transform active:scale-[0.97] disabled:opacity-40',
              disabled: submitting,
            }}
          >
            📷 Photograph
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-[2px] border-2 border-threat bg-threat px-4 py-[17px] font-display text-[16px] text-chalk transition-transform active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-40"
          >
            💣 Plant
          </button>
        </div>
