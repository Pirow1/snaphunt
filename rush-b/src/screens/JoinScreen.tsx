  ...
  return (
    <main className="flex h-full w-full flex-col bg-void text-smoke">
      <TopBar title="Join Operation" back="/" right={
        <span className="rounded-[2px] border border-threat/40 bg-threat/10 px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.15em] text-threat">Defuser</span>
      } />

      <div className="flex flex-1 flex-col gap-7 overflow-y-auto px-5 py-6">
        <section>
          <label className="block font-mono text-[10px] uppercase tracking-[0.25em] text-ash mb-2">
            Operation Code
          </label>
          <input
            type="text"
            ...
            autoComplete="off"
            spellCheck={false}
            className="w-full rounded-[2px] border-2 border-rim bg-surface px-4 py-3 font-mono text-[24px] tracking-[0.4em] text-spark placeholder:text-ash/40 focus:border-spark focus:outline-none text-center"
          />
        </section>
        ...
        <section>
          ...
          <input
            ...
            spellCheck={false}
            autoFocus
            className="w-full rounded-[2px] border-2 border-rim bg-surface px-4 py-3 font-mono text-[20px] tracking-widest text-ink placeholder:text-ash/50 focus:border-spark focus:outline-none"
          />
        </section>

        <section>
          <span className="block font-mono text-[10px] uppercase tracking-[0.25em] text-ash mb-2">Insignia</span>
        ...
        {error && (
          <div className="rounded-[2px] border border-threat/50 bg-threat/10 px-3 py-2 font-mono text-[12px] text-threat">
            {error}
          </div>
        )}
