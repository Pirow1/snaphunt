  ...
  return (
    <main className="relative flex h-full w-full flex-col bg-void">
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
        ...
        <div className="mb-6 inline-block border-2 border-threat px-4 py-1 font-mono text-[10px] uppercase tracking-[0.35em] text-threat">
          ⚠ Classified Operation
        </div>

        {/* Main title */}
        <div className="font-display text-[90px] leading-[0.85] tracking-[0.02em] text-chalk">
          RUSH
        </div>
        <div className="font-display text-[90px] leading-[0.85] tracking-[0.02em] text-spark">
          B
        </div>

        <div className="mt-4 font-mono text-[12px] uppercase tracking-[0.3em] text-ash">
        ...
        <div className="mt-8 flex gap-8 border-y border-rim py-4">
          {[['ROUNDS', '3'], ['TIMER', '3–15'], ['DEFUSE', 'PUZZLE']].map(([label, val]) => (
            <div key={label} className="text-center">
              <div className="font-display text-[26px] text-spark">{val}</div>
