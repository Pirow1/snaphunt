...
export default function ResultScreen({ outcome, onContinue }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className={`font-display text-[90px] leading-none ${defused ? 'text-clear' : 'text-threat'}`}>
        {defused ? '✓' : '💥'}
      </div>

      <div>
        <div className={`font-display text-[52px] tracking-[0.06em] ${defused ? 'text-clear' : 'text-threat'}`}>
          {defused ? 'DEFUSED' : 'DETONATED'}
        </div>
        <div className="mt-2 font-mono text-[12px] uppercase tracking-[0.2em] text-ash">
          {defused ? 'Bomb successfully neutralised.' : 'The bomb has detonated. Planter wins.'}
        </div>
      </div>

      <div className="font-mono text-[10px] uppercase tracking-[0.15em] text-ash/60 animate-blink">
        Tap to continue
      </div>
