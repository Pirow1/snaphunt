  ...
  return (
    <main className="flex h-full w-full flex-col items-center justify-center bg-void px-6 text-center gap-6">
      ...
      <div className={`transition-all duration-500 ${show ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`}>
        <div className="font-mono text-[11px] uppercase tracking-[0.35em] text-ash mb-3">
          Your role this round
        </div>
        <div className={`font-display text-[72px] leading-none ${isPlanter ? 'text-threat' : 'text-spark'}`}>
          {isPlanter ? '💣' : '🔍'}
        </div>
        <div className={`font-display text-[44px] tracking-[0.06em] mt-2 ${isPlanter ? 'text-threat' : 'text-spark'}`}>
          {isPlanter ? 'PLANTER' : 'DEFUSER'}
        </div>
        <div className="mt-3 font-mono text-[12px] text-ash max-w-[240px] mx-auto leading-relaxed">
          {isPlanter
