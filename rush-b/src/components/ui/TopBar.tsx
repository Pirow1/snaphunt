    ...
    <div className="flex items-center justify-between border-b-2 border-rim bg-carbon/90 px-[22px] py-3 backdrop-blur-md">
      {back ? (
        <button
          ...
          aria-label="Back"
          onClick={() => go(back)}
          className="flex h-9 w-9 items-center justify-center rounded-[2px] border border-rim bg-surface font-mono text-[18px] text-smoke active:bg-spark active:text-void"
        >
          ←
        </button>
      ) : (
      ...
      <div className="font-display text-[18px] tracking-[0.1em] text-chalk">{title}</div>
...
export function TopBarBadge({ children, tone = 'yellow' }: { children: ReactNode; tone?: 'yellow' | 'red' | 'green' }) {
  ...
  return (
    <span className={`${cls} rounded-[2px] border px-2.5 py-1 font-mono text-[11px] uppercase tracking-[0.12em]`}>
      {children}
    </span>
  );
}
