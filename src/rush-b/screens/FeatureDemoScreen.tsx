...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mb-8">
      ...
      <div className="font-rb-display text-[18px] tracking-[0.1em] text-ash uppercase mb-4 px-5">{title}</div>
...
function SoundBtn({ label, onClick }: { label: string; onClick: () => void }) {
  ...
  return (
    <button
      ...
      className={`rounded-[2px] border px-3 py-2 font-rb-mono text-[11px] uppercase tracking-[0.1em] transition-colors
  ...
  return (
    <main className="min-h-full w-full bg-void text-smoke pb-12">
      ...
      <div className="px-5 py-4">
        <div className="font-rb-mono text-[10px] uppercase tracking-[0.3em] text-ash">Dev</div>
        <div className="font-rb-display text-[36px] tracking-[0.06em] text-spark">FEATURE DEMO</div>
      </div>
      ...
      <Section title="Proximity Heartbeat">
        <div className="grid grid-cols-4 gap-2">
          {DIST_PRESETS.map((d) => (
            <button
              ...
              className={`rounded-[2px] border px-2 py-2 font-rb-mono text-[10px] uppercase tracking-[0.08em] transition-colors
        ...
        <div className={`mt-3 rounded-[2px] border ${activeDist === null ? 'border-rim' : 'border-spark/40'} bg-surface px-4 py-3 flex items-center gap-4`}>
          ...
          <div className="flex-1">
            {activeDist === null ? (
              <span className="font-rb-mono text-[11px] text-ash">Select a distance above to start</span>
            ) : heartbeatBand ? (
              <>
                <div className="font-rb-mono text-[11px] text-smoke">{activeDist}m → every {heartbeatBand.intervalMs}ms</div>
                <div className="font-rb-mono text-[10px] text-ash mt-0.5">vol {Math.round(heartbeatBand.volume * 100)}%</div>
              </>
            ) : (
              <span className="font-rb-mono text-[11px] text-ash">{activeDist}m — out of range ({'>'} 100m)</span>
            )}
          </div>
          {activeDist !== null && (
            <button type="button" onClick={() => setActiveDist(null)} className="font-rb-mono text-[10px] text-ash/60 underline">stop</button>
          )}
        </div>
      </Section>
      ...
      <Section title="Bomb Timer">
        <div className="grid grid-cols-4 gap-2">
          {[{ label: '1 min', s: 60 }, { label: '30 sec', s: 30 }, { label: '10 sec', s: 10 }, { label: 'Reset', s: 0 }].map(({ label, s }) => (
            <button
              ...
              className="rounded-[2px] border border-rim bg-surface px-2 py-2 font-rb-mono text-[10px] uppercase tracking-[0.08em] text-ash hover:border-spark/60 hover:text-smoke transition-colors"
            >
              {label}
            </button>
          ))}
        ...
        {timerExpired && (
          <div className="mt-2 font-rb-mono text-[11px] text-threat text-center uppercase tracking-[0.15em]">
      ...
      <Section title="Bomb Card — Sharpen Levels">
        ...
        <div className="font-rb-mono text-[10px] text-ash/60 text-center mt-1">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[{ label: '3m' }, { label: '25m' }, { label: '80m' }, { label: '150m' }, { label: '500m' }, { label: 'No GPS' }].map(({ label }) => (
            <button
              ...
              type="button"
              onClick={() => {}}
              className="rounded-[2px] border border-rim bg-surface px-2 py-2 font-rb-mono text-[10px] uppercase tracking-[0.08em] text-ash"
            >
              {label}
            </button>
          ))}
      ...
      <Section title="Defusal Puzzle">
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex-1 rounded-[2px] border-2 border-spark bg-spark/10 py-3 font-rb-display text-[20px] tracking-[0.06em] text-spark active:scale-[0.97] transition-transform"
          >
            💣 OPEN PUZZLE
          </button>
          <div className={`font-rb-mono text-[13px] uppercase tracking-[0.15em] min-w-[90px] text-center
            ${puzzleResult === 'DEFUSED ✓' ? 'text-clear' : puzzleResult === 'FAILED ✗' ? 'text-threat' : 'text-ash'}`}>
            {puzzleResult}
          </div>
        </div>
        <div className="font-rb-mono text-[10px] text-ash/60">
