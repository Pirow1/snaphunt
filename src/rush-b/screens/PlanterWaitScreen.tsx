    ...
    <main className="flex h-full w-full flex-col bg-void text-smoke">
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-6 px-5">
        ...
        <div className="w-full rounded-[2px] border border-rim bg-surface px-4 py-4 text-center">
          <div className="font-rb-display text-[48px] leading-none">💣</div>
          <div className="mt-2 font-rb-display text-[22px] tracking-[0.06em] text-threat">BOMB IS LIVE</div>
          <div className="mt-2 font-rb-mono text-[11px] uppercase tracking-[0.15em] text-ash">
            Defusers are hunting your location
          </div>
        </div>

        <div className="w-full rounded-[2px] border border-rim bg-surface px-4 py-3">
          <div className="font-rb-mono text-[10px] uppercase tracking-[0.2em] text-ash mb-2">Bomb Position</div>
          {round?.planter_lat != null ? (
            <div className="font-rb-mono text-[12px] text-smoke">
              {round.planter_lat.toFixed(5)}, {round.planter_lng!.toFixed(5)}
            </div>
          ) : (
            <div className="font-rb-mono text-[11px] text-ash">Pinned</div>
          )}
        </div>
      </div>
    </main>
