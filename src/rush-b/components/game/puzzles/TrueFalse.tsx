      ...
      <div className="grid w-full grid-cols-2 gap-3">
        {([true, false] as const).map((val) => (
          <button
            ...
            className={`rounded-[3px] border-[1.5px] py-4 font-rb-display text-[18px] font-extrabold uppercase tracking-[0.08em] transition-colors duration-100 ${btnClass(val)}`}
          >
            {val ? '✓ True' : '✗ False'}
          </button>
        ))}
