...
export function ShapeSortPuzzle({ puzzle, onSolve, onFail }: Props) {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <p className="font-rb-display text-[16px] font-bold uppercase tracking-[0.1em] text-parchment">
        {puzzle.prompt}
      </p>
      ...
      {!done && (
        <button
          ...
          className="w-full rounded-[3px] border-[1.5px] border-gold bg-gold py-3 font-rb-display text-[14px] font-extrabold uppercase tracking-[0.08em] text-forest-dark disabled:opacity-40"
        >
          Confirm Selection
        </button>
      )}
