  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      {puzzle.visual?.type === 'flash' && showFlash && (() => {
        const flash = puzzle.visual as { type: 'flash'; emoji: string; count: number };
        return (
          <div className="flex flex-wrap justify-center gap-2 rounded-[4px] border border-gold/20 bg-forest-mid/60 p-4">
            {Array.from({ length: flash.count }, (_, i) => (
              <span key={i} className="text-3xl">{flash.emoji}</span>
            ))}
          </div>
        );
      })()}
      ...
      {(!showFlash) && (
        <p className="text-center font-rb-display text-[18px] font-bold uppercase tracking-[-0.02em] text-parchment">
          {puzzle.prompt}
        </p>
      )}
          ...
          {puzzle.options.map((opt) => {
            ...
            return (
              <button
                ...
                disabled={revealed}
                className={`rounded-[3px] border-[1.5px] py-3.5 font-rb-display text-[15px] font-bold tracking-wide transition-colors duration-100 ${cls}`}
              >
                {opt}
              </button>
            );
