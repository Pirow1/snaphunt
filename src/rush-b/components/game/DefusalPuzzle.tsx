  ...
  return (
    <div className={`fixed inset-0 z-50 flex flex-col bg-void`}>
      ...
      <div className="border-b-2 border-rim bg-carbon px-5 py-3">
        <div className="font-rb-mono text-[10px] uppercase tracking-[0.25em] text-ash">Defusal Sequence</div>
        <div className="font-rb-display text-[22px] tracking-[0.05em] text-threat">SOLVE TO DEFUSE</div>
      </div>
      ...
      <div className={`flex flex-1 flex-col border-2 ${overlay} m-4 rounded-[2px] overflow-hidden transition-colors duration-300`}>
        {solved && (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 p-6">
            <div className="font-rb-display text-[60px] text-clear animate-stamp-in">✓</div>
            <div className="font-rb-display text-[28px] tracking-[0.1em] text-clear">DEFUSED</div>
          </div>
        )}
        {failed && (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 p-6">
            <div className="font-rb-display text-[60px] text-threat">✗</div>
            <div className="font-rb-display text-[22px] tracking-[0.1em] text-threat">FAILED — RETRY</div>
          </div>
        )}
        {!solved && !failed && (
